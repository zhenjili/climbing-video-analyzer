...
def process_video_task(self, video_path: str, file_id: str) -> dict:
    ...
    if analysis.improvement_frames:
        ...
        saved = processor.save_frames_at_timestamps(
            video_path, timestamps, pose_frames,
            str(settings.output_dir), file_id,
        )
        for frame in analysis.improvement_frames:
            if frame.time_sec in saved:
                frame.image_url = f"/outputs/{saved[frame.time_sec]}"
